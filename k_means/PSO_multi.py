...
#import os
#import ffmpeg
#ffpath = os.path.dirname(ffmpeg.__file__)
...
def makeHeatMap(name,iterb,itere,param_name,param_max,data):
  pts = data[iterb]
  for i in range (iterb+1,itere):
      pts = np.vstack((pts,data[i]))
  
  heatmap, xedges, yedges = np.histogram2d(pts[:,0]/float(param_max[0]), pts[:,1]/float(param_max[1]), bins=(20,20))
  extent = [xedges[0], xedges[-1], yedges[0], yedges[-1]]
  plt.figure(figsize=(10,10))
  plt.imshow(heatmap.T, extent=extent, origin='lower')
  plt.title(name)
  plt.xlabel(param_name[0])
  plt.ylabel(param_name[1])
  plt.axis('square')
  plt.colorbar()
  #plt.show()
...
parser.add_argument('--heatmap',dest='heatmap',type=bool,default=False)
...
param_name = ['max_depth','reg_alpha','min_child_weight','subsample','colsample_bytree','gamma','reg_lambda']
...
optimizer = GlobalBestPSO(n_particles=args.particles, dimensions=n_dimensions, options=options, bounds=bounds)
cost, pos = optimizer.optimize(f_BDT, iters=args.iters, data=score_km_train_norm, labels=labels_train_np, param_list=param_name, n_processes=4)
...
d = Designer(limits=[(0,50), (0,5), (0,1)],
             label=param_name)
...
if(args.heatmap):
  makeHeatMap("allIter", 0, n_iters, parameter_name, param_max_list, optimizer.pos_history)
  makeHeatMap("FirstTenIter", 0, 10, parameter_name, param_max_list, optimizer.pos_history)
  makeHeatMap("LastTenIter", n_iters-10, n_iters, parameter_name, param_max_list, optimizer.pos_history)
...
keys += "\n"
f_result.write(keys)
value = "{0},{1},{2},{3}".format(args.c1,args.c2,args.w,cost)
for pos_ in pos:
    value = value + ",{}".format(pos_)
value += "\n"
f_result.write(value)
f_result.close()

position_array = np.array(optimizer.pos_history)
pos_array = position_array.reshape([n_iters*n_particles,len(param_name)])
np.savetxt("pos_history.csv", pos_array, delimiter=',')
